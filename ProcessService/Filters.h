...
class CCubicFilter : public CGenericFilter {
public:
    CCubicFilter (double dWidth = double(1.0)) : CGenericFilter(dWidth) {}
    ...
    {
        dVal = fabs(dVal); 
        return (dVal < 1.0 ? (2.0 * dVal - 3.0) * dVal * dVal + 1.0 : 0.0); 
    }
};

class CBellFilter : public CGenericFilter {
public:
    CBellFilter (double dWidth = double(1.5)) : CGenericFilter(dWidth) {}
    virtual ~CBellFilter() {}
...
public:
    ...
    CBsplineFilter (double dWidth = double(2.0)) : CGenericFilter(dWidth) {}
    ...
    double Filter (double dVal) 
    {
        ...
        if (dVal < 1.0) {
			tt = dVal * dVal;
			return((.5 * tt * dVal) - tt + (2.0 / 3.0));
		} else if (dVal < 2.0) {
			dVal = 2.0 - dVal;
			return((1.0 / 6.0) * (dVal * dVal * dVal));
		}
		return(0.0);   
    }
};
