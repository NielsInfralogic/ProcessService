...
TCHAR	g_szServiceName[MAX_PATH] = _T("ProcessService");
TCHAR	g_szServiceDescription[MAX_PATH] = _T("PDFhub Process Service");
...
BOOL InstallService()
{
	...
	if (schSCManager == NULL) {
		g_util.LogprintfResample(_T("Couldn't open service manager"));
		return FALSE;
	}
	...
	if (dwFLen == 0 || dwFLen == MAX_PATH) {
		g_util.LogprintfResample(_T("GetModuleFileName failed"));
		return FALSE;
	}
	...
	if (schService == NULL) {
		...
		g_util.LogprintfResample(_T("Couldn't create service"));
		return FALSE;
	}

	if (::StartService(schService, NULL, 0) == FALSE) {
		::CloseServiceHandle(schSCManager);
		g_util.LogprintfResample(_T("Couldn't start service"));
		return FALSE;
	}
	...
	keyClasses.Create(HKEY_LOCAL_MACHINE, _T("Software\\InfraLogic\\ProcessService"));
	...
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\ProcessService\\Config.ini"), _T("ConfigFile"));
...
BOOL DeleteService()
{
	...
	if (hService == NULL) {
		::CloseServiceHandle(schSCManager);
		g_util.LogprintfResample(_T("Couldn't open service"));
		return FALSE;
	}
	...
	g_util.LogprintfResample(_T("Service could not be deleted"));
	return FALSE;
}
...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	strcpy(szEventStr, "ProcessService");
	...
	if (g_prefs.LoadPreferencesFromRegistry() == FALSE)
		g_util.LogprintfResample("CANNOT READ Registry: Software\\InfraLogic\\InputService\\Parameters..\r\n");
	else {
		g_util.LogprintfResample("INFO: Registry read: Software\\InfraLogic\\InputService\\Parameters..\r\n");
	...
	if (dbmaint.InitDB(g_prefs.m_DBserver, g_prefs.m_Database, g_prefs.m_DBuser, g_prefs.m_DBpassword, g_prefs.m_IntegratedSecurity, sErrorMessage) == FALSE) {
		g_util.LogprintfResample("Unable to connect to database - %s", sErrorMessage);
	...
	if (dbmaint.LoadConfigIniFile(g_prefs.m_instancenumber, sIniFile, sIniFileConvert, g_prefs.m_setupEnableXMLpath, sErrorMessage))
	...
	if (sIniFile != "") {
		...
		g_util.LogprintfResample("INFO: Ini config file %s read:\r\n", sIniFile);
		LogEvent(EVENTLOG_SUCCESS, IDS_INILOADED, szEventStr, NULL, NULL);
	}

	if (sIniFileConvert != "") {
		g_prefs.LoadIniFileConvert(sIniFileConvert);

		g_util.LogprintfConvert("INFO: Ini config file %s read:\r\n", sIniFileConvert);
