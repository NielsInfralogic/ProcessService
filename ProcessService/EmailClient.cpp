...
BOOL CEmailClient::TestPOP3Connection(CString sPOP3Server, int nPOP3Port, CString sPOP3Username, CString sPOP3Password, TCHAR *szErrorMessage)
{
	...
	if (success != true) {
		util.LogprintfConvert(_T("POP3 ERROR: Error connecting to server %s - %s"), (LPCSTR)sPOP3Server, (LPCSTR)mailman.lastErrorText());
	...
	if (numMessages == -1) {
		util.LogprintfConvert(_T("POP3 ERROR: Error getting mail count - %s"), mailman.lastErrorText());
	...
	CString sDestFolder, EMAILFILEINFOSTRUCT aFileList[], int nMaxFiles, TCHAR *szErrorMessage)
{
	...
	bool success = mailman.UnlockComponent(_T("INFRAL.CB1092019_4czEsHZS615x"));
	if (success != true) {
		util.LogprintfConvert(_T("POP3 ERROR: Error connecting to server %s - %s"), sPOP3Server, mailman.lastErrorText());
		return -1;
	}
	...
	if (bundle == NULL) {
		util.LogprintfConvert(_T("POP3 ERROR: Error copying mail to local storage - %s"), mailman.lastErrorText());
		//mailman.Pop3EndSession();
		return -1;
	}
	CkString sUIDL;
		...
		if (bPassed) {
			for (int j = 0; j <= n - 1; j++) {
				...
				if (email->SaveAttachedFile(j, sDestFolder) == false) {
					util.LogprintfConvert(_T("POP3 ERROR: Error saving attachment %s to folder %s - %s"), sFileName, sDestFolder, mailman.lastErrorText());
					continue;
				}
	...
	CString sMailCC, CString sMailSubject, CString sMailBody, BOOL bHtmlBody, CString sAttachmentFile)
{
	...
	bool success = mailman.UnlockComponent(_T("INFRAL.CB1092019_4czEsHZS615x"));
	if (success != true) {
		util.LogprintfConvert(_T("POP3 ERROR: Error connecting to server %s - %s"), sSMTPServer, mailman.lastErrorText());
		return FALSE;
	}
	...
	if (sAttachmentFile != _T("")) {
		...
		if (contentType == NULL) {
			util.LogprintfConvert(_T("SMTP WARNING: Unable to add attachment file %s - %s"), sAttachmentFile, email.lastErrorText());
			return FALSE;
		}
	}
	...
	if (success != true) {
		util.LogprintfConvert(_T("SMTP ERROR: Unable to connect to smtp server %s - return code %s"), sSMTPServer, mailman.lastErrorText());
		return FALSE;
	}
	else
		util.LogprintfConvert(_T("SMTP INFO:  E-mail '%s' sent to %s with attachment %s"), sMailSubject, sMailTo, sAttachmentFile);

	success = mailman.CloseSmtpConnection();
	if (success != true) {
		util.LogprintfConvert(_T("SMTP ERROR: Connection to SMTP server not closed cleanly"));
		return FALSE;
	}
